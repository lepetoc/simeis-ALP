...
class Game:
    ...
    def init_game(self):
        ...
        if self.pla["solid"]:
            ...
            self.modtype = "GasSucker"

        # Ensure the ship has a corresponding module, buy one if we don't
        if not check_has(ship["modules"], "modtype", self.modtype):
            self.buy_first_mining_module(self.modtype, self.sta, self.sid)
...
if __name__ == "__main__":
    ...
    # TODO Add condition check to buy upgrades
    while True:
        print("")
        game.disp_status()
        game.go_mine(game.pla)
        game.disp_status()
        game.go_sell()
...
# 2.Scanner a partir de la station
#     - Déterminer la planète la plus proche
#     - Stocker si elle est gazeuse ou solide

# 3.Acheter le vaisseau
#     - Acheter l'équipage
#     - Acheter le module qui correspond a la planète

# 4.Lancer expedition vers la planète
#     - Attendre d'y arriver
#     - Miner
#     - Attendre que le cargo du vaisseau soit plein
#     - Retourner a la station

# 5.A la station
#     - Décharger le vaisseau
#     - Refuel et repair
#     - Vendre petit a petit
#     - Quand assez d'argent, arrêter de vendre
#     - Acheter upgrade du trader
#     - Continuer de vendre
#     - Quand assez d'argent, acheter amélioration pilote ou operator
#     - Retourner a l'étape 4

# 6.Optionnel
#     - Verifier si assez d'argent pour acheter nouveau vaisseau complet (équipage, module)
